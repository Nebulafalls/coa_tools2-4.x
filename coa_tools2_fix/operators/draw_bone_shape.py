...
class COATOOLS2_OT_DrawBoneShape(bpy.types.Operator):
    bl_idname = "coa_tools2.draw_bone_shape"
    bl_label = "Create Bone Shape"

    bone_shapes: EnumProperty(
        name="Bone Shapes",
        description="List of all custom Bone Shapes.",
        items=get_bone_shapes,
    )
    ...
    def invoke(self, context, event):
        ...
        if context.active_pose_bone != None:
            ...
            if context.active_object.type == "ARMATURE" and (
                context.active_pose_bone.custom_shape != None
                or shape_name in bpy.data.objects
            ):
                shape_name = (
                    context.active_pose_bone.custom_shape.name
                    if context.active_pose_bone.custom_shape != None
                    else context.active_pose_bone.name + "_custom_shape"
                )
    ...
    def execute(self, context):
        if (
            ...
            if self.bone_shapes == "NEW_SHAPE":
                bpy.ops.coa_tools2.edit_mesh(mode="DRAW_BONE_SHAPE")
            else:
                shape_name = context.active_pose_bone.name + "_custom_shape"
                if shape_name not in bpy.data.objects:
                    bone_shape = bpy.data.meshes.new_from_object(
                        bpy.data.objects[self.bone_shapes],
                        preserve_all_data_layers=False,
                        depsgraph=None,
                    )
                    bone_shape.name = shape_name
                bpy.ops.coa_tools2.edit_mesh(
                    mode="DRAW_BONE_SHAPE", new_shape_name=self.bone_shapes
                )
        else:
            self.report({"WARNING"}, "Select Bone in Pose Mode.")
        return {"FINISHED"}
