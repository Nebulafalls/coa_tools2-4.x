...
class COATOOLS2_OT_EditWeights(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        sprite_object = bpy.data.objects[self.sprite_object_name]
        if (
            get_armature(sprite_object) == None
            or get_armature(sprite_object) not in context.visible_objects
        ):
        ...
        sprite_object.coa_tools2.edit_weights = True
        sprite_object.coa_tools2.edit_mode = "WEIGHTS"
    ...
    def draw_coords(
        ...
        if len(indices) > 0:
            batch = batch_for_shader(
                shader, draw_type, {"pos": coords}, indices=indices
            )
        else:
        ...
        if obj != None:
            ...
            for i, vert in enumerate(me.vertices):
                if not vert.hide:
                    ...
                    segment_weight = (
                        weight - segment_length * color_index
                    ) / segment_length
                    if weight < 1.0:
                        final_color = colorband[color_index].lerp(
                            colorband[color_index + 1], segment_weight
                        )
                    else:
