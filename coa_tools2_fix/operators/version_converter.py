...
class COATOOLS2_OT_VersionConverter(bpy.types.Operator):
    bl_idname = "coa_tools2.convert_deprecated_data"
    ...
    def convert_sprite_objects(self):
        ...
        for obj in bpy.data.objects:
            obj_name = obj.name
            if "sprite_object" in obj.coa_tools2 and obj.type != "ARMATURE" and obj not in converted_sprite_objects:
                armature = None
                for child in obj.children:
                    if child.type == "ARMATURE":
                        armature = child
                        armature.coa_tools2["sprite_object"] = True
                        armature["coa_tools2"] = obj["coa_tools2"]
                        break
                for child in obj.children:
                    if child != armature:
    ...
    def convert_properties(self):
        ...
        for mesh in bpy.data.meshes:
            for prop_name in mesh.keys():
                if "coa_" in prop_name and "coa_tools2" not in prop_name:
                    prop_name_new = prop_name.split("coa_")[1]
                    mesh.coa_tools2[prop_name_new] = mesh[prop_name]
        ...
        for obj in bpy.data.objects:
            ...
            for prop_name in list(obj.keys()):
                if "coa_" in prop_name and "coa_tools2" not in prop_name:
                    prop_name_new = prop_name.split("coa_")[1]
                    obj.coa_tools2[prop_name_new] = obj[prop_name]
                    del obj[prop_name]
        ...
        for obj in bpy.data.objects:
            if obj.animation_data != None:
                ...
                for driver in anim_data.drivers:
                    if "coa_" in driver.data_path and not "coa_tools2." in driver.data_path:
                        driver.data_path = "coa_tools2." + driver.data_path.split("coa_")[1]
    ...
    def execute(self, context):
        ...
        self.set_shading()
        context.scene.coa_tools2.deprecated_data_found = False
        return {"FINISHED"}
