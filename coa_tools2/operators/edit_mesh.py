...
class COATOOLS2_TO_DrawPolygon(bpy.types.WorkSpaceTool):
    ...
    bl_icon = os.path.join(functions.get_coa_tools2_dir(), "icons", "coa_tools2.draw_polygon")
    bl_widget = None
    # bl_keymap = (
...
class COATOOLS2_OT_DrawContour(bpy.types.Operator):
    ...
    def get_projected_point(self,edge,custom_pos=None,disable_edge_threshold=False):
        ...
        dist = min((v1-v2).magnitude * .5 , context.scene.coa_tools2.snap_distance * context.space_data.region_3d.view_distance)
    ...
    def get_edge_slide_points(self,context,bm):
        ...
        snap_distance = context.scene.coa_tools2.snap_distance * context.space_data.region_3d.view_distance
    ...
    def snap_to_edge_or_vert(self,coord, get_bm_obj = False):
        ...
        distance = scene.coa_tools2.snap_distance * bpy.context.space_data.region_3d.view_distance
        snap_coord = coord
        point_type = None
        bm_obj2 = None
    ...
    def modal(self, context, event):
        ...
        try:
            ...
            ### leave edit mode
            if context.active_object == None or (context.active_object != None and context.active_object.type == "MESH" and context.active_object.mode != "EDIT" and not self.draw_handler_removed) or self.sprite_object.coa_tools2.edit_mesh == False or context.active_object.type != "MESH":
            ...
            if self.in_view_3d and context.active_object != None and self.type not in ["MIDDLEMOUSE"] and self.sprite_object.coa_tools2.edit_mesh and click_button not in [select_button]:
                ### set click drag    
                ...
                if not self.ctrl:
                    if self.mouse_press and self.inside_area:
                        if self.cur_distance > context.scene.coa_tools2.distance:
                            i = int(self.cur_distance / (context.scene.coa_tools2.distance))
                            for j in range(i):
                                new_vertex_pos = (self.cursor_pos_hist + (self.draw_dir * context.scene.coa_tools2.distance))
    ...
    def execute(self, context):
        try:
            bpy.utils.register_tool(COATOOLS2_TO_DrawPolygon, after={"builtin.select"}, separator=True, group=True)
        except:
            pass
    ...
    def execute(self, context):
        ...
        for edge in bm.edges:
            if edge.select:
                ...
                scene.coa_tools2.distance = ((edge.verts[0].co - edge.verts[1].co)).magnitude/mult# edge.calc_length()/mult        
        bmesh.update_edit_mesh(obj.data)    
        return {"FINISHED"}
