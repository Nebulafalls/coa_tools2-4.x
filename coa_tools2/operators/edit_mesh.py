...
def clean_boundary_edges(bm, obj):
    ...
    for edge in bm.edges:
        if edge.calc_length() < edges_len_average * 0.12 and not edge.tag:
            edges.append(edge)
...
class COATOOLS2_OT_ReprojectSpriteTexture(bpy.types.Operator):
    bl_idname = "coa_tools2.reproject_sprite_texture"
    ...
    def reproject(self, context):
        ...
        hide_base_sprite = obj.data.coa_tools2.hide_base_sprite
        ...
        bmesh.update_edit_mesh(obj.data)
        obj.data.coa_tools2.hide_base_sprite = hide_base_sprite
    ...
    def cleanup_and_fill_mesh(self, obj, bm):
        ...
        for e1 in bm.edges:
            for e2 in bm.edges:
                ...
                edges_share_points = e1.verts[0].co.xz in [
                    e2.verts[0].co.xz,
                    e2.verts[1].co.xz,
                ] or e1.verts[1].co.xz in [e2.verts[0].co.xz, e2.verts[1].co.xz]
        ...
        for vert in bm.verts:
            if not vert.hide and len(vert.link_edges) == 0:
                ...
                for vert2 in bm.verts:
                    ...
                    if (
                        vert != vert2 and vert2.co != vert.co
        ...
        if len(vert_loops) > 0:
            for i, loop in enumerate(vert_loops):
                for vert in loop:
                    exclude_verts = (
                        [vert.link_edges[0].other_vert(vert), vert]
                    ...
                    if len(vert.link_edges) == 1:
                        for j, loop_next in enumerate(vert_loops):
                            for vert2 in loop_next:
                                if vert2 != vert:
                                    edge1 = (
                                        vert.co - vert.link_edges[0].other_vert(vert).co
                                    ).normalized()
                                    ...
                                    if edge1.length > 0 and edge2.length > 0:
                                        ...
                                        if (
                                            (vert.co - vert2.co).magnitude < distance
                                            and vert2 not in exclude_verts
                                            and abs(angle) > 30
                                            and not edge_is_intersecting(
                                                [vert, vert2], bm
                                            )
                                        ):
                                            distance = (vert.co - vert2.co).magnitude
        ...
        for edge in bm.edges:
            if (
                not edge.hide
                and edge.is_wire
                and not edge.verts[0].hide
                and not edge.verts[1].hide
            ):
                delete_edges.append(edge)
        bmesh.ops.delete(bm, geom=delete_edges, context="EDGES")
...
class COATOOLS2_OT_DrawContour(bpy.types.Operator):
            ...
            if not face.hide:
                ...
                for edge in face.edges:
                    if (
                        ...
                        and not edge.is_wire
                        and not edge.is_boundary
                    ):
                        invalid_edges += 1
    ...
    def draw_verts(self, context, obj, bm, position, use_snap=False):
        ...
        intersect_prev_vert = (
            bm.select_history[0] if len(bm.select_history) > 0 else None
        )
        for i, p in enumerate(self.intersection_points):
            ...
            if sub_edge != None:
                divider = (
                    obj.matrix_world @ sub_edge.verts[0].co
                    - obj.matrix_world @ sub_edge.verts[1].co
                ).magnitude
                if divider != 0:
                    percentage = (
                        obj.matrix_world @ sub_edge.verts[0].co - c
                    ).magnitude / (
                        obj.matrix_world @ sub_edge.verts[0].co
                        - obj.matrix_world @ sub_edge.verts[1].co
                    ).magnitude
    ...
    def get_projected_point(self, edge, custom_pos=None, disable_edge_threshold=False):
        ...
        dist = min(
            (v1 - v2).magnitude * 0.5,
            context.scene.coa_tools2.snap_distance
    ...
    def get_edge_slide_points(self, context, bm):
        ...
        for edge in bm.edges:
            ...
            if not edge.hide:
                ...
                if (
                    self.mouse_pos_3d.x > left
                    and self.mouse_pos_3d.x < right
                    and self.mouse_pos_3d.z < top
                    and self.mouse_pos_3d.z > bottom
                ):
    ...
    def get_visible_verts(self, context, bm):
        ...
        active_vert = None
        if (
            len(bm.select_history) > 0
            and type(bm.select_history[0]) == bmesh.types.BMVert
        ):
    ...
    def snap_to_edge_or_vert(self, coord, get_bm_obj=False):
        ...
        distance = (
            scene.coa_tools2.snap_distance
        ...
        if self.selected_vert_coord != None and coord != None:
            ...
            for edge in bm.edges:
                if not edge.hide:  # and (edge.is_wire or edge.is_boundary):
                    e2 = [
                        (obj.matrix_world @ edge.verts[0].co).xz,
                    ...
                    if ip != None:
                        ...
                        if (ip - self.selected_vert_coord).magnitude > 0.001 and (
                            ip - coord
                        ).magnitude > 0.001:
    ...
    def delete_geometry(self, context, bm, position, single_vert=False):
        obj = context.active_object
        snapped_vert_coord, point_type, bm_ob = (
            self.snapped_vert_coord,
            self.point_type,
            self.bm_objs,
        )
    ...
    def modal(self, context, event):
        ...
        try:
            ...
            if self.type_prev in ["G", "S", "R"] or (
                self.value_prev in ["CLICK", "PRESS"] and self.value == "RELEASE"
            ):
            ...
            if (
                ...
                if scene.coa_tools2.surface_snap:
                    ...
                    (
                        ...
                        self.point_type,
                        self.bm_objs,
                        self.verts_edges_data,
                    ) = self.snap_to_edge_or_vert(self.mouse_pos_3d)
                else:
                    (
                        self.snapped_vert_coord,
                        self.point_type,
                ...
                if (
                    ...
                    if not self.alt:
                        ...
                        self.draw_verts(
                            context, obj, bm, self.cursor_pos_hist, use_snap=True
                        )
                    return {"RUNNING_MODAL"}

                if event.value == "RELEASE" and (
                    event.type == "MOUSEMOVE" or event.type == "LEFTMOUSE"
                ):
                    self.mouse_press = False
                ...
                if not self.ctrl:
                    if self.mouse_press and self.inside_area:
                        if self.cur_distance > context.scene.coa_tools2.distance:
                            i = int(
                                self.cur_distance / (context.scene.coa_tools2.distance)
                            )
                            for j in range(i):
                                new_vertex_pos = self.cursor_pos_hist + (
                                    self.draw_dir * context.scene.coa_tools2.distance
                                )
                                self.draw_verts(
                ...
                if self.type_prev in ["G"] and self.contour_length > 0:
                    if (
                        len(select_history) > 0
                        and type(select_history[0]) == bmesh.types.BMVert
                    ):
                        self.selected_vert_coord = (
                            obj.matrix_world @ select_history[0].co
                        )
                ...
                scene.tool_settings.double_threshold = scene.coa_tools2.snap_distance
                ### delete verts
                if (
                    self.alt
                    and (self.click_drag or self.type == click_button)
                    and not self.type in ["MIDDLEMOUSE"]
                ):
                    self.selected_vert_coord = None
                    self.contour_length = 0
                ...
                if self.shift and self.point_type == "EDGE":
                    ...
                    if self.type == click_button:
                        ...
                        scene.coa_tools2.distance = length
                ...
                if (
                    (
                        self.selected_verts_count == 0
                        and self.selected_vert_coord != None
                    )
                    or self.type in [select_button]
                    or (self.ctrl and self.type in ["Z"])
    ...
    def exit_edit_mode(self, context, event, error=False):
        ...
        context.view_layer.objects.active = (
            bpy.data.objects[self.edit_object_name]
            if self.edit_object_name in bpy.data.objects
            else None
        )
        context.scene.coa_tools2.view = self.prev_coa_view
        if context.active_object != None:
            bpy.ops.object.mode_set(mode="EDIT")
        ...
        if self.mode == "DRAW_BONE_SHAPE":
            ...
            context.scene.coa_tools2.lock_to_bounds = self.draw_bounds
            if self.armature != None:
            ...
            if len(context.active_object.data.vertices) > 4:
                context.active_object.data.coa_tools2.hide_base_sprite = True
            bpy.ops.object.mode_set(mode="OBJECT")
    ...
    def prepare_edit_object(self, context):
        ...
        texture_preview_object.location[1] += 0.1
    ...
    def finish_edit_object(self, context):
        ...
        try:
            bpy.utils.unregister_tool(COATOOLS2_TO_DrawPolygon)
        except:
            pass

        self.texture_preview_object = (
            bpy.data.objects[self.texture_preview_object_name]
    ...
    def execute(self, context):
        ...
        if context.active_object == None or (
            context.active_object.type != "MESH" and self.mode != "DRAW_BONE_SHAPE"
        ):
            self.report(
        ...
        if (
            self.texture_preview_object != None
            and self.texture_preview_object.type == "MESH"
        ):
            self.texture_preview_object.active_shape_key_index = 0
            self.texture_preview_object.data.coa_tools2.hide_base_sprite = False
        ...
        if self.mode == "DRAW_BONE_SHAPE":
            self.draw_bounds = context.scene.coa_tools2.lock_to_bounds
            context.scene.coa_tools2.lock_to_bounds = False
    ...
    def draw_callback_px(self):
        ...
        if obj != None and obj.mode == "EDIT":
            ...
            if len(self.bounds) > 3:
                vec1 = self.coord_3d_to_2d(Vector(self.bounds[0]) + y_offset) + Vector(
                    (-1, -1)
                )
                vec2 = self.coord_3d_to_2d(Vector(self.bounds[1]) + y_offset) + Vector(
                    (-1, 1)
                )
                vec3 = self.coord_3d_to_2d(Vector(self.bounds[3]) + y_offset) + Vector(
                    (1, 1)
                )
                vec4 = self.coord_3d_to_2d(Vector(self.bounds[2]) + y_offset) + Vector(
                    (1, -1)
                )
...
class COATOOLS2_OT_PickEdgeLength(bpy.types.Operator):
    bl_idname = "coa_tools2.pick_edge_length"
    ...
    def execute(self, context):
        ...
        for edge in bm.edges:
            if edge.select:
                # scene.coa_tools2.distance = (obj.matrix_world @ (edge.verts[0].co - edge.verts[1].co)).magnitude/mult# edge.calc_length()/mult
                scene.coa_tools2.distance = (
                    (edge.verts[0].co - edge.verts[1].co)
                ).magnitude / mult  # edge.calc_length()/mult
        bmesh.update_edit_mesh(obj.data)
        return {"FINISHED"}
