...
class COATOOLS2_OT_ShowHelp(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        for area in context.screen.areas:
            if area.type == "VIEW_3D":
                ...
                if not wm.coa_tools2.show_help and round(self.alpha_current,1) == 0:#event.type in {"RIGHTMOUSE", "ESC"}:
                    return self.finish()
    ...
    def draw_callback_px(self):
        ...
        texts += text_headline
        if self.sprite_object == None or (self.sprite_object != None and self.sprite_object.coa_tools2.edit_mode in ["OBJECT"]):
            texts += text_general
        if self.sprite_object == None or (self.sprite_object != None and self.sprite_object.coa_tools2.edit_mode in ["OBJECT"]):
            texts += text_blender
        if self.sprite_object == None or (self.sprite_object != None and self.sprite_object.coa_tools2.edit_mode in ["ARMATURE"]):
            texts += text_armature
        if self.sprite_object == None or (self.sprite_object != None and self.sprite_object.coa_tools2.edit_mode in ["SHAPEKEY"]):
            texts += text_shapekey
        if self.sprite_object == None or (self.sprite_object != None and self.sprite_object.coa_tools2.edit_mode in ["WEIGHTS"]):
            texts += text_weights
        if self.sprite_object == None or (self.sprite_object != None and self.sprite_object.coa_tools2.edit_mode in ["MESH"]):
            texts += text_mesh
