...
class COATOOLS2_UL_JsonImport(bpy.types.UIList):
    def draw_item(
...
class COATOOLS2_OT_CreateMaterialGroup(bpy.types.Operator):
    bl_idname = "coa_tools2.create_material_group"
    ...
    def create_coa_material_group(self):
        ...
        if CONSTANTS.COA_NODE_GROUP_NAME in bpy.data.node_groups:
            ...
            group_tree = bpy.data.node_groups.new(
                CONSTANTS.COA_NODE_GROUP_NAME, "ShaderNodeTree"
            )
...
class COATOOLS2_OT_LoadJsonData(bpy.types.Operator):
    bl_idname = "coa_tools2.load_json_data"
    ...
    mode: EnumProperty(
        items=(
            ("UPDATE", "Update Existing", "Update", "FILE_REFRESH", 0),
    ...
    def execute(self, context):
        ...
        if "nodes" in sprite_data:
            for i, sprite in enumerate(sprite_data["nodes"]):
                if (
                    sprite["name"] in self.json_import_data
                    and self.json_import_data[sprite["name"]].active
                ):
                    ...
                    if self.mode == "UPDATE":
                        if sprite["name"] in context.scene.objects:
                            ...
                            bpy.ops.coa_tools2.reimport_sprite(
                                filepath=filepath,
                                name=sprite["name"],
                                scale=scale,
                                pos=pos,
...
class COATOOLS2_OT_ImportSprite(bpy.types.Operator):
    bl_idname = "coa_tools2.import_sprite"
    ...
    def create_mesh(
        self, context, name="Sprite", width=100, height=100, pos=Vector((0, 0, 0))
    ):
    ...
    def create_material(self, context, mesh, name="Sprite"):
        ...
        bpy.ops.coa_tools2.create_material_group()
        ...
        node_tree.links.new(
            coa_node.inputs["Texture Color"],
            tex_node.outputs["Color"],
            verify_limits=True,
        )
        node_tree.links.new(
            coa_node.inputs["Texture Alpha"],
            tex_node.outputs["Alpha"],
            verify_limits=True,
        )
        node_tree.links.new(
            coa_node.outputs["BSDF"], output_node.inputs["Surface"], verify_limits=True
        )
    ...
    def execute(self, context):
        if os.path.exists(self.path):
            ...
            obj.coa_tools2.z_value = int(-self.pos[1])
...
class COATOOLS2_OT_ImportSprites(bpy.types.Operator, ImportHelper):
    bl_idname = "coa_tools2.import_sprites"
        ...
        if ext not in [".json"]:
            for i in self.files:
                ...
                if not self.replace or i.name not in bpy.data.objects:
                    ...
                    bpy.ops.coa_tools2.reimport_sprite(
                        filepath=filepath, name=i.name, reposition=False
                    )
        else:
            ...
            bpy.ops.coa_tools2.load_json_data(
                "INVOKE_DEFAULT", json_data=str(sprite_data), filepath=self.filepath
            )
...
class COATOOLS2_OT_ReImportSprite(bpy.types.Operator, ImportHelper):
    bl_idname = "coa_tools2.reimport_sprite"
    ...
    pos: FloatVectorProperty(default=Vector((0, 0, 0)))
    scale: FloatProperty(default=0.01)
    offset: FloatVectorProperty(default=Vector((0, 0, 0)))
    ...
    def execute(self, context):
        ...
        if self.name in bpy.data.objects:
            ...
            if self.reposition:
                obj.location = (
                    Vector((pos[0], pos[1], -pos[2])) * self.scale
                    + Vector((self.offset[0], self.offset[1], self.offset[2]))
                    * self.scale
                )
