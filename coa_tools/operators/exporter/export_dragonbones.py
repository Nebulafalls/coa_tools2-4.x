...
def get_skin_slot(self, sprite, armature, scale, slot_data=None):
    ...
    if sprite_data_name in img_names:
        tex_path = os.path.join(
            self.scene.coa_tools.project_name + "_texture", img_names[sprite_data_name]
        )
            ...
            for i in range(anim.frame_end + 1):
                ...
                for slot in self.sprites:
                    if slot.type == "MESH":
                        ...
                        if property_key_on_frame(
                            slot, ["coa_tools.alpha", "coa_tools.modulate_color"], frame
                        ):
                            keyframe_data = {}
                        ...
                        if property_key_on_frame(
                            slot, ["coa_tools.slot_index"], frame
                ...
                if armature != None:
                    for j, bone in enumerate(armature.data.bones):
                        ...
                        bake_anim = (
                            self.scene.coa_tools.export_bake_anim
                            and frame % self.scene.coa_tools.export_bake_steps == 0
                        )
...
class COATOOLS_OT_DragonBonesExport(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        texture_dir_path = os.path.join(
            export_path, self.scene.coa_tools.project_name + "_texture"
        )
        json_path = os.path.join(
            export_path, self.scene.coa_tools.project_name + "_ske.json"
        )
        ...
        if self.scene.coa_tools.export_image_mode == "ATLAS":
            ...
            if len(sprites) > 0:
                generate_texture_atlas(
                    self,
                    sprites,
                    self.scene.coa_tools.project_name,
                    export_path,
        ...
        self.json_data["armature"] = [
            setup_armature_data(self.sprite_object, self.scene.coa_tools.armature_name)
