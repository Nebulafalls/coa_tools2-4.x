...
class COATOOLS_PT_Info(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if context.scene.coa_tools.deprecated_data_found:
            row = layout.row()
            row.operator(
                "coa_tools.convert_deprecated_data", icon="LIBRARY_DATA_BROKEN"
            )

        if (
            context.space_data.shading.type != "RENDERED"
            or context.scene.view_settings.view_transform != "Standard"
        ) and not context.scene.coa_tools.deprecated_data_found:
            row = layout.row()
            row.operator(
                "coa_tools.change_shading_mode",
        ...
        #     op.text = "Check out CutoutAnimation Tools Addon for Blender by Aodaruma."
...
class COATOOLS_PT_ObjectProperties(bpy.types.Panel):
    ...
    def draw_outliner(self, context, layout, sprite_object, scene):
        ...
        row.prop(
            context.scene.coa_tools, "outliner_filter_names", text="", icon="VIEWZOOM"
        )
        if context.scene.coa_tools.outliner_favorites:
            row.prop(
                context.scene.coa_tools, "outliner_favorites", text="", icon="SOLO_ON"
            )
        else:
            row.prop(
                context.scene.coa_tools, "outliner_favorites", text="", icon="SOLO_OFF"
            )

        row = col.row()
        row.template_list(
            "COATOOLS_UL_Outliner",
            "",
            scene.coa_tools,
            "outliner",
            scene.coa_tools,
    ...
    def draw(self, context):
        ...
        if sprite_object != None and obj != None:
            ...
            if (
                ...
                row = layout.row(align=True)
                row.prop(
                    obj.data.coa_tools, "hide_base_sprite", text="Hide Base Sprite"
                )
                if (
                    len(obj.data.vertices) > 4
                    and obj.data.coa_tools.hide_base_sprite == False
                ):
                    row.prop(
                        obj.data.coa_tools,
                        "hide_base_sprite",
            ...
            if obj != None and obj.type == "MESH" and obj.mode == "OBJECT":
                ...
                if obj.coa_tools.type == "SLOT" and len(obj.coa_tools.slot) > 0:
                    ...
                    op = row.operator(
                        "coa_tools.add_keyframe", text="", icon="HANDLETYPE_ALIGNED_VEC"
                    )
                    op.prop_name = "coa_tools.slot_index"
            ...
            if obj != None and obj.type == "MESH" and obj.mode == "OBJECT":
                ...
                op = row.operator(
                    "coa_tools.add_keyframe", text="", icon="KEYTYPE_MOVING_HOLD_VEC"
                )
...
class COATOOLS_PT_Tools(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if obj != None and sprite_object != None:
            ...
            if (
                sprite_object.coa_tools.edit_mesh == False
                and sprite_object.coa_tools.edit_shapekey == False
                and sprite_object.coa_tools.edit_armature == False
                and sprite_object.coa_tools.edit_weights == False
                and obj.mode not in ["SCULPT"]
            ):
                row = layout.row(align=True)
                op = row.operator(
                    "coa_tools.edit_mesh", text="Edit Mesh", icon="GREASEPENCIL"
                )
                op.mode = "EDIT_MESH"
        ...
        no_edit_mode_active = (
            sprite_object != None
            and sprite_object.coa_tools.edit_shapekey == False
            and sprite_object.coa_tools.edit_mesh == False
            and sprite_object.coa_tools.edit_armature == False
            and sprite_object.coa_tools.edit_weights == False
        )
        ...
        if obj == None or (
            ...
            row = layout.row(align=True)
            row.operator(
                "coa_tools.create_sprite_object",
        ...
        if (
            ...
            and "sprite_object" not in obj.coa_tools
        ):
            row = layout.row(align=True)
            row.operator(
                "coa_tools.define_sprite_object",
        ...
        if sprite_object != None:
            ...
            if (
                ...
                if sprite_object != None and (
                    ...
                    row = layout.row()
                    row.operator(
                        "coa_tools.change_alpha_mode",
                        text="Change Alpha Mode",
                        icon="SHADING_RENDERED",
                    )
                    row = layout.row()
                    row.operator(
                        "coa_tools.change_texture_interpolation_mode",
        ...
        if (
            ...
            and obj.mode == "EDIT"
            and obj.type == "MESH"
            and sprite_object.coa_tools.edit_mesh
        ):
            row = layout.row(align=True)
            ...
            operator = row.operator(
                "coa_tools.generate_mesh_from_edges_and_verts",
                text="Generate Mesh",
...
class COATOOLS_UL_AnimationCollections(bpy.types.UIList):
    def draw_item(
        ...
        if context.scene.coa_tools.nla_mode == "NLA" and item.name not in [
            "NO ACTION",
            "Restpose",
        ]:
...
class COATOOLS_UL_EventCollection(bpy.types.UIList):
    def draw_item(
        ...
        op.index = index
        op = row.operator(
            "coa_tools.remove_timeline_event", text="", icon="PANEL_CLOSE", emboss=False
        )
        op.index = index
        if not item.collapsed:
            for i, event in enumerate(item.event):
                ...
                if (
                    ...
                    or context.scene.coa_tools.runtime_format == "CREATURE"
                ):
    ...
    def draw(self, context):
        ...
        if sprite_object != None:
            ...
            box_col.template_list(
                "COATOOLS_UL_AnimationCollections",
                "dummy",
                sprite_object.coa_tools,
                "anim_collections",
                sprite_object.coa_tools,
            ...
            col.operator(
                "coa_tools.remove_animation_collection", text="", icon="REMOVE"
            )

            if (
                len(sprite_object.coa_tools.anim_collections) > 2
                and sprite_object.coa_tools.anim_collections_index > 1
            ):
                col.operator(
            ...
            if (
                len(sprite_object.coa_tools.anim_collections) > 0
                and sprite_object.coa_tools.anim_collections[
