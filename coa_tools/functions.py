...
def draw_sculpt_ui(self, context, layout):
    ...
    if obj != None and obj.mode == "SCULPT":
        ...
        if obj.data.shape_keys != None and len(obj.data.shape_keys.key_blocks) > 0:
            op = subrow.operator(
                "coa_tools.shapekey_rename", icon="OUTLINER_DATA_FONT", text=""
            )
...
def set_action(context, item=None):
    ...
    if item == None:
        index = min(
            len(sprite_object.coa_tools.anim_collections) - 1,
            sprite_object.coa_tools.anim_collections_index,
        )
    ...
    for child in animation_objects:
        ...
        if child.type == "MESH" and item.name == "Restpose":
            ...
            and context.scene.coa_tools.nla_mode == "ACTION"
        ):
...
def display_children(self, context, obj):
    ...
    if context.scene.coa_tools.active_collection in bpy.data.collections:
        active_collection = bpy.data.collections[
            context.scene.coa_tools.active_collection
        ]
    if obj != None:
        layout = self.layout
        col = layout.column(align=True)
        ...
        disable_list = (
            sprite_object.coa_tools.edit_mesh
            or sprite_object.coa_tools.edit_armature
            or sprite_object.coa_tools.edit_weights
            or sprite_object.coa_tools.edit_shapekey
        )

        for obj2 in context.view_layer.objects:
            if get_sprite_object(obj2) == obj2:
                in_range = current_display_item in range(
                    context.scene.coa_tools.display_page
    ...
    if sprite_object != None and sprite_object.coa_tools.show_children:
        ...
        for i, child in enumerate(children):
            if child.name in active_collection.objects:
                ...
                name_found = (
                    obj.coa_tools.filter_names.upper() in child.name.upper()
                    and not obj.coa_tools.filter_names == ""
                )
                current_display_item += 1
                if in_range or context.scene.coa_tools.display_all or name_found:
                    if (
                        (sprite_object.coa_tools.favorite and child.coa_tools.favorite)
                        or not sprite_object.coa_tools.favorite
                        ...
                        if obj.coa_tools.filter_names.upper() in child.name.upper():
                            ...
                            if (
                                child.type == "MESH"
                                and child.coa_tools.type == "SLOT"
                                and child.coa_tools.slot_show
                            ):
                            ...
                            if (
                                child.type == "MESH"
                                and sprite_object.coa_tools.edit_armature
                            ):
                                op = subrow3.operator(
                                    "coa_tools.bind_mesh_to_bones",
                                    text="",
